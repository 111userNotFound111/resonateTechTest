...
class PromotionCode {
    ...
    constructor(storeId, transactionId){
        this.storeId = storeId
        this.transactionId = transactionId
        this.encodeList = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
        this.encodeListLength = this.encodeList.length
        this.millisecondToDay = 1000 * 60 * 60 *24
    }

    // execute class using following execution function for testing
    static main(storeId, transactionId){
        const PromoCode = new PromotionCode(storeId, transactionId)
        // generate promotion code
        const promoCode = PromoCode.generateShortCode()
        // decode promotion code
        const decodedCode = PromoCode.decodeShortCode(promoCode)
        return {
            promoCode,
            decodedCode
        }
    ...
    // date decompress 
    dateDeconvert(dateInput){
        const date = dateInput * millisecondToDay
        return date
    }

    // code slices
    codeSlicer(codeInput,start, end){
